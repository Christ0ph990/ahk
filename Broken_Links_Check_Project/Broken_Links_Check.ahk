...
SetTitleMatchMode, 2
#Include %A_ScriptDir%
#Include C:\Users\Barnesc\Desktop\Broken_Links_Check_Project\funcs.ahk
SetFormat, Float, 0.1
;{ ------------------ Stupid Shit -------------------------
Loop, 27
{
...
FileInstall, C:\Users\Barnesc\Desktop\Broken_Links_Check_Project\curl.exe, %A_ScriptDir%\AKN_Extract\curl.exe, 1
FileInstall, C:\Users\Barnesc\Desktop\Broken_Links_Check_Project\jq.exe, %A_ScriptDir%\AKN_Extract\jq.exe, 1
FileInstall, C:\Users\Barnesc\Desktop\Broken_Links_Check_Project\tidy.exe, %A_ScriptDir%\AKN_Extract\tidy.exe, 1
FileInstall, C:\Users\Barnesc\Desktop\Broken_Links_Check_Project\tidyconfig.txt, %A_ScriptDir%\AKN_Extract\tidyconfig.txt, 1
Progress, 0, Downloading product information., , Downloading
URLDownloadToFile, http://help-staging.autodesk.com/view/, %A_ScriptDir%\AKN_Extract\prodcodes.txt
...
TitleObj := Object()
HrefObj := Object()
CloseAnchorObj := Object()
LinkBrokeObj := Object()
BrokenLinkCounter := 0
;}+++++++++++++++++++++++++++++

;{ ********* Grab product codes from staging area ***************
Loop, Read, %A_ScriptDir%\AKN_Extract\prodcodes.txt
{
...
Loop % ProductCode.length()
{
	...
	{
		ProdCodeVar := ProductCode[A_Index]
		URLDownloadToFile, http://help-staging.autodesk.com/view/%ProdCodeVar%/, %A_ScriptDir%\AKN_Extract\selectedproduct.txt
		SelectedProduct := ProdCodeVar
	}
}

;{ ********* Grab 1st year or language code from staging area ***************
Loop, Read, %A_ScriptDir%\AKN_Extract\selectedproduct.txt
{
	If InStr(A_LoopReadLine, "align")
	{
		If InStr(A_LoopReadLine, "folder.gif")
...
Loop % AYearLanCode.length()
{
	...
	{
		AYearLanVar := AYearLanCode[A_Index]
		URLDownloadToFile, http://help-staging.autodesk.com/view/%SelectedProduct%/%AYearLanVar%/, %A_ScriptDir%\AKN_Extract\selectedyearlan1.txt
		SelectedAYearLan := AYearLanVar
	}
}

;{ ********* Grab 2st year or language code from staging area ***************
Loop, Read, %A_ScriptDir%\AKN_Extract\selectedyearlan1.txt
{
	If InStr(A_LoopReadLine, "align")
	{
		If InStr(A_LoopReadLine, "folder.gif")
...
Gui, ProdStag: Add, Button, h30 w70 gHREFbit, OK
...
Loop, Read, %A_ScriptDir%\AKN_Extract\ParsedJSON.txt
{
	...
	{
		;IfInString, A_LoopReadLine, FCAM-WhatsNew
		;{
...
Progress, Off
FileAppend, GUID¬Title¬Status¬On or near this text¬Link¬`r`n, %A_ScriptDir%\AKN_Extract\output.txt
ProgressCount :=
EstimateTime :=
FinalGuid :=
FinalTitle :=
...
Loop % GuidhtmObj.length()
{
	...
	URLDownloadToFile, http://%StagingOrProduction%.autodesk.com%GuidhtmVar%, %A_ScriptDir%\AKN_Extract\htmfile%A_Index%.txt
	RunWait, %comspec% /c %A_ScriptDir%\AKN_Extract\tidy.exe -config `"%A_ScriptDir%\AKN_Extract\tidyconfig.txt`" `"%A_ScriptDir%\AKN_Extract\htmfile%A_Index%.txt`", , Hide
	Loop, Read, %A_ScriptDir%\AKN_Extract\htmfile%A_Index%.txt
	{	ErrorLevel = 0
		2HrefArray := 0
		FileDelete, %A_ScriptDir%\AKN_Extract\downloadcheck.txt
		IfInString, A_LoopReadLine, `<body ID`=`"
		{
			StringSplit, 2GuidArray, A_LoopReadLine, `"
			FinalGuid := 2GuidArray2		
		}
		...
		IfInString, A_LoopReadLine, `<a HREF`=`"
		{
			StringSplit, HrefArray, A_LoopReadLine, `"
			StringReplace, 2HrefArray, HrefArray2, `&amp`;, `&, All
			HrefObj.insert(2HrefArray)
			IfInString, 2HrefArray, `.htm
			{
				LinkBrokeObj.insert("OK: Valid link within publication")
				continue
			}
			IfInString, 2HrefArray, http
			{
				ErrorLevel = 0
				URLDownloadToFile, %2HrefArray%, %A_ScriptDir%\AKN_Extract\downloadcheck.txt
				If(ErrorLevel = 1)
					{
						LinkBrokeObj.insert("Invalid URL: No response from website")
						BrokenLinkCounter += 1
						ErrorLevel = 0
						continue
					}
				Loop, Read, %A_ScriptDir%\AKN_Extract\downloadcheck.txt
				{
					
					IfInString, A_LoopReadLine, null`(`{`"totalResult`"`:`"0`"`,`"`_status`"`:`"Completed`"`}
					{
						LinkBrokeObj.insert("404: Broken inter/intra-book GUID")
						BrokenLinkCounter += 1
						;continue
						break
					}
					IfInString, A_LoopReadLine, `<link rel`=`"canonical`" href`=`"http`:`/`/www`.autodesk`.com`/404`"`/`>
					{
						LinkBrokeObj.insert("404: Broken MURL")
						BrokenLinkCounter += 1
						;continue
						break
					}
					IfInString, A_LoopReadLine, `<title`>Help`<`/title`> ;needs soemthing else
					{
						LinkBrokeObj.insert("OK: Valid help MURL")
						;continue
						break
					}
					IfInString, A_LoopReadLine, `<meta name`=`"language`"
					{
						LinkBrokeObj.insert("OK: Valid link between publications")
						;continue
						break
					}
					IfInString, A_LoopReadLine, `<`/html`>
					{
						LinkBrokeObj.insert("OK: Valid external link")
						;continue
						break
					}
		...
		{
			StringSplit, 1CloseAnchorArray, A_LoopReadLine, `>
			StringTrimRight, 2CloseAnchor, 1CloseAnchorArray2, 3
			StringReplace, 3CloseAnchor, 2CloseAnchor, `&gt`;, `>, All
			StringReplace, 4CloseAnchor, 3CloseAnchor, `&amp`;, `&, All
			CloseAnchorObj.insert(4CloseAnchor)
		}
	}	
	Progress, %PercentDone%, %PercentDone% `%`r`nTime Remaining: %EstimateTime%`r`n------------------------------------------------------------------------------------------------------------------------------------------`r`nGUID`:`r`n%FinalGuid%`r`n`r`nTitle`:`r`n%FinalTitle%`r`n`r`nLink`:`r`n[%FinalHref%`],, %PercentDone% `%
	...
	Loop % HrefObj.length()
	{
		FinalHref := HrefObj[A_Index]
		FinalCloseAnchor := CloseAnchorObj[A_Index]
		FinalLinkBroke := LinkBrokeObj[A_Index]
		FileAppend, ¬¬%FinalLinkBroke%¬%FinalCloseAnchor%¬%FinalHref%`r`n, %A_ScriptDir%\AKN_Extract\output.txt
		Progress, %PercentDone%, %PercentDone% `%`r`nTime Remaining: %EstimateTime%`r`n------------------------------------------------------------------------------------------------------------------------------------------`r`nGUID`:`r`n%FinalGuid%`r`n`r`nTitle`:`r`n%FinalTitle%`r`n`r`nLink`:`r`n[%FinalHref%`],, %PercentDone% `%
	}
	HrefObj := ""
	HrefObj := Object()
...
Sleep, 500
Progress, Off
MsgBox, 4, Complete ,Report finished in %TotalTime%.`n`nThere are %BrokenLinkCounter% broken links.`n`nWould you like to export the results to excel?
IfMsgBox, Yes
{
Excelify()
Sleep, 500
